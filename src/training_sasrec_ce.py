...
# Dùng chung cache — đảm bảo cùng data split để so sánh công bằng với BERT4Rec và MetaBERT4Rec
...
# CE Loss — điểm khác biệt chính của SASRec+ so với SASRec gốc (dùng BCE)
# Theo bài báo 3604915.3610644: CE với full softmax cho tín hiệu huấn luyện
# mạnh hơn nhiều so với BCE với 1 negative sample
...
def validate_one_epoch(model, val_loader, device, val_type, epoch, K_list=[1, 5, 10]):
    model.eval()
    metrics = {f"{m}@{k}": 0.0 for m in ["Recall", "NDCG", "MRR"] for k in K_list}
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Validating {val_type}"):
            ...
            # Lấy ID của phim đúng (thường là phim cuối cùng trong sequence)
            # Giả sử dataset của Hải trả về target_item_id trong batch
            target_items = batch["label"][:, -1].to(device) 

            # 🔥 FULL RANKING: Đặt candidates=None để lấy logits cho toàn bộ vocab
            ...
            # Lấy logits tại vị trí thời gian cuối cùng dự báo cho tương lai
            logits = logits[:, -1, :] # [B, Vocab_size]

            # Tính hạng của phim đúng trong toàn bộ Vocab
            # Dùng topk hoặc sort để tìm rank. Cách tối ưu:
            target_scores = logits.gather(1, target_items.unsqueeze(1)) # Điểm của phim đúng
            # Đếm xem có bao nhiêu phim có điểm cao hơn phim đúng
            ...
            for K in K_list:
                ...
                metrics[f"NDCG@{K}"] += torch.where(hit > 0, 1.0 / torch.log2(ranks_float + 1), torch.zeros_like(hit)).sum().item()
                metrics[f"MRR@{K}"] += torch.where(ranks <= K, 1.0 / ranks_float, torch.zeros_like(ranks_float)).sum().item()
...
def train_one_epoch(model, batch, accumulation_steps):
    ...
    with torch.amp.autocast('cuda'):
        ...
        # Dùng token_mask từ dataset (giống BERT4Rec) để tránh OOM
        ...
        # Guard: nếu không có vị trí nào được mask thì bỏ qua batch này
        if y_pred.shape[0] == 0:
            return 0.0
