...
# 1. Import mô hình SASRecF (SASRec + Early Fusion Genre)
from model import SASRecF, SASRecF_Concat # Nhớ import model mới

d_model = 16       # Giảm từ 64 xuống 16
n_layers = 1       # Giảm từ 2 xuống 1
num_epochs = 3     # Chỉ chạy 3 epoch
...
num_epochs, val_iter, patience = 3, 1, 5
# num_epochs, val_iter, patience = 50, 1, 5
accum_steps = 8

# Đổi tên thư mục để không bị ghi đè lên SASRec gốc
experiment_dir = f"../data/sasrec_f_{d_model}_timesplit"
...
# ─── 3. INITIALIZE ──────────────────────────────────────────────────────────
# Khởi tạo SASRecF với tham số num_genres=20
model = SASRecF(max_len=200, num_genres=20, d_model=d_model, n_heads=n_heads, n_layers=n_layers, vocab_size=vocab_size).to(device)
...
for epoch in range(start_epoch, num_epochs + 1):
    # CHÚ Ý: Đã bật is_meta=True để feed Genre vào mô hình
