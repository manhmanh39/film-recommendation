       ...
       # CSS tùy chỉnh để biến nút bấm thành Logo
        ...
        if not df.empty:
            ...
            # ==========================================
                ...
                if history_df.empty:
                    st.markdown("### 🔥 Top Phim Thịnh Hành")
                    results = df.head(12) 
                else:
                    ...
                    if 'recommended_movies' not in st.session_state:
                        st.session_state['recommended_movies'] = df.sample(16) # Tăng lên 16 phim cho đẹp lưới 4 cột
                ...
                for index, row in results.reset_index().iterrows():
                    with cols[index % 4]:
                        ...
                        with st.container(border=True):
                            # GỌI HÀM VÀ HIỂN THỊ ẢNH Ở ĐÂY
                            poster_url = fetch_poster(row['tmdbId'])
                            st.image(poster_url, use_container_width=True)
                            
                            # Thiết lập chiều cao cố định cho tiêu đề để các thẻ bằng nhau
